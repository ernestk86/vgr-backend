...
curl -s -u admin:$jenkins_pass $new_dns/crumbIssuer/api/json | jq -r '.crumb' >> $jenkins_crumb_path
jenkins_crumb=$(<$jenkins_crumb_path)

sudo rm $jenkins_token_path
curl "$new_dns/me/descriptorByName/jenkins.security.ApiTokenProperty/generateNewToken" --data 'newTokenName=jenkins-token' -u admin:$jenkins_pass -H "Jenkins-Crumb:$jenkins_crumb" | jq -r '.tokenValue' >> $jenkins_token_path
